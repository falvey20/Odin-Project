...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      subject(:game_not_over) {described_class.new(8, '6') }
      # remove the 'x' before running this test
      it 'is not game over' do
        expect(game_not_over.game_over?).to be(false)
      end
    end
  end
  ...
  describe '#final_message' do
    ...
    context 'when count is 2-3' do
      subject(:game_success) { described_class.new(7, '7', 3) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        congrats_phrase = "Congratulations! You picked the random number in 3 guesses!\n"
        expect { game_success.final_message }.to output(congrats_phrase).to_stdout
      end
    end
    ...
    context 'when count is 4 and over' do
      subject(:game_lose) { described_class.new(7, '7', 4) }
      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        lose_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_lose.final_message }.to output(lose_phrase).to_stdout
      end
    end
  end
end
