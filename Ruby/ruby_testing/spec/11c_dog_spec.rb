...
describe Dog do
  ...
  subject(:Snoopy) { described_class.new('Snoopy', 'Dalmatian')}
  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when talk is called on Dog' do
    it 'responds WOOF!' do
      expect(subject.talk).to eq('WOOF!')
    end
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(subject.barking?).to eq(false)
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(subject.sleeping?).to eq(true)
  end
end
